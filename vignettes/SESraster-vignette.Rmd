...
Species distributions are dynamic and ever-changing, exhibiting variations both across time and space. To study and understand the complex mechanisms underlying species distributions, null models are often used. A null model is a simplified representation of how species distributions would look if specific ecological processes or factors were not operating. The package [`SESraster`](https://github.com/HemingNM/SESraster) covers a current gap by offering functions for randomization of presence and absence rasters with or without including spatial structure on randomized patterns of species distribution.


## Installation {#install}

The development version of `SESraster` can be downloaded from [GitHub](https://github.com/HemingNM/SESraster). If you find any bug, let us know through [SESraster Issues](https://github.com/HemingNM/SESraster/issues). To install the package, run the following code:
...
The package `SESraster` uses the R package `terra` (version \>= 1.6) (Hijmans, 2022) as dependency. Once installed, package can be loaded into R using `library()`:
...
The `SESraster` package implements functions to randomize spatial data with or without including spatial structure through the functions `bootspat_str()` and `bootspat_naive()`, respectively. The function `bootspat_naive()` randomizes a raster stack according to the observed frequency of presences for each species (layer) using the methods: sites (raster cells), species (raster layers) or both (layers and cells). This is the most commonly used method of randomization for community data.
The function `bootspat_str()` randomizes a raster stack keeping the species richness fixed across raster cells. This method is less commonly seen in the literature.
