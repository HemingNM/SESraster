...
## Standardized effect size (SES) {#ses}
...
## Calculating SES {#analysis}
### Random species generation {#randsp}
Now, let's see some of the package features. First, we will create some
...
function `appmean()` through `FUN` argument. We also randomize the
...
Notice that NAs can be ignored by using `FUN_args = list(na.rm = TRUE)`:
...
In this second example it is performed a trait sampling **with** replacement.
